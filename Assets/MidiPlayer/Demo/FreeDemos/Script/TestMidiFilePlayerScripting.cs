...
namespace MidiPlayerTK
{
    public class TestMidiFilePlayerScripting : MonoBehaviour
    {
        ...
        public bool IsWaitNotesOff = false;
        ...
        public float DelayStartStop = 2000f;
        ...
        public void StartPlay(string name)
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                if (starttick > 0f)
                    midiFilePlayer.MPTK_TickCurrent = (long)((float)midiFilePlayer.MPTK_TickLast * (starttick / 100f));
            }
        ...
        void OnGUI()
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                //Avoid slider with ticks position trigger
                GUILayout.BeginHorizontal();
                GUILayout.Label($"Tick Start Position: {starttick} %", myStyle.TitleLabel3, GUILayout.Width(300));
                starttick = (long)GUILayout.HorizontalSlider(starttick, 0f, 100f, GUILayout.Width(150));
                GUILayout.EndHorizontal();
...
#if MPTK_PRO
                GUILayout.BeginHorizontal(GUILayout.Width(500));
                if (GUILayout.Button(new GUIContent($"Play Ramp-Up {(int)DelayStartStop} ms", "")))
                    midiFilePlayer.MPTK_Play(DelayStartStop);
                if (GUILayout.Button(new GUIContent($"Stop Downward {(int)DelayStartStop} ms", "")))
                    midiFilePlayer.MPTK_Stop(DelayStartStop);
                GUILayout.Label("Delay", myStyle.TitleLabel3, GUILayout.Width(50));
                DelayStartStop = GUILayout.HorizontalSlider(DelayStartStop, 0f, 5000f, myStyle.SliderBar, myStyle.SliderThumb, GUILayout.Width(120));
                GUILayout.EndHorizontal();
#endif
