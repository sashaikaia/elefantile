        ...
        void OnGUI()
        {
            ...
            if (midiStreamPlayer != null)
            {
                ...
                if (GUILayout.Button("Play", myStyle.BtStandard, GUILayout.Width(buttonWidth * 0.666f)))
                {
                    ...
                    StopOneNote();
                    // Play infinite note (duration is set to -1)
                    PlayOneInfiniteNote();
                }
                ...
                // ------------------------------
                GUILayout.BeginVertical(myStyle.BacgDemos1);
                GUILayout.Label("Loop on Notes and Presets", myStyle.TitleLabel3);
                ...
                GUILayout.BeginHorizontal(GUILayout.Width(350));
                Frequency = Slider("Frequency", Frequency, 0.01f, 1f);
...
//#if MPTK_PRO
                ...
                {
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("Range from Lib", myStyle.TitleLabel3, GUILayout.Width(widthFirstCol));
...
//#endif
        ...
        private float Slider(string title, float val, float min, float max, bool alignright = false, float wiLab = 100, float wiSlider = 150)
        {
            float ret;
            GUILayout.BeginHorizontal();
        ...
        /// Play one shot
        /// </summary>
        private void PlayOneInfiniteNote()
        {
            //Debug.Log($"{StreamChannel} {midiStreamPlayer.MPTK_ChannelPresetGetName(StreamChannel)}");
            ...
            NotePlaying = new MPTKEvent()
            {
                ...
                Value = CurrentNote,
                Channel = StreamChannel,
                Duration = -1, // millisecond, -1 to play undefinitely
                Velocity = Velocity, // Sound can vary depending on the velocity
                Delay = Convert.ToInt64(NoteDelay * 1000f),
            };
#if MPTK_PRO
