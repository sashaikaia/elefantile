    ...
    public partial class fluid_voice
    {
        ...
        static int[] list_of_weakgenerators_to_initialize =
     {
        ...
        public void fluid_voice_start(MPTKEvent note)
        {
            ...
            // A single tick represents one hundred nanoseconds or one ten-millionth of a second.
            // There are 10,000 ticks in a millisecond, or 10 million ticks in a second. 
            TimeAtStart = note.Delay * fluid_voice.Nano100ToMilli + DateTime.Now.Ticks;
                ...
                case (int)fluid_gen_type.GEN_VOLENVRELEASE:             /* SF2.01 section 8.1.3 # 38 */
                    {
                        ...
                        if (synth.VerboseCalcGen) Debug.Log(string.Format("Calc {0} x={1:F7} count={2} ms. incr={3:F7} fluid_tc2sec_release(x)={4:F7}",
...
#define fluid_phase_set_int(a, b)    ((a) = ((unsigned long long)(b)) << 32)
            ...
            float[] dsp_reverb_buf, float[] dsp_chorus_buf)
        {
            ...
            //     (System.DateTime.Now.Ticks - TimeAtStart) / fluid_voice.Nano100ToMilli, 
            ...
            else
            {
                ...
                //Debug.LogFormat("t:{0} calc amp_max:{1:F7} volenv_val:{2:F7} section:{3} ({4}) amplitude_that_reaches_noise_floor:{5:F7} ",
                //    (System.DateTime.Now.Ticks - TimeAtStart) / fluid_voice.Nano100ToMilli,
                //   amp_max, volenv_val, volenv_section, (int)volenv_section, amplitude_that_reaches_noise_floor);


                /* And if amp_max is already smaller than the known amplitude,
                 * which will attenuate the sample below the noise floor, then we
                 * can safely turn off the voice. Duh. */
                if (amp_max <= synth.CutOffVolume)
                {
                    //Debug.Log(amp_max + " " + amplitude_that_reaches_noise_floor);
                    //fluid_profile(FLUID_PROF_VOICE_RELEASE, ref);
                    fluid_voice_off();
                    goto post_process;
                }
            ...
            if (synth.VerboseVolume)
                Debug.LogFormat("Volume - [{0,4}] TimeFromStart:{1} Delta:{2:F2} section:{3} amp_max:{4:F2} target_amp:{5:F2} amp:{6:F2} reaches_noise:{7:F2}",
                  ...
                  amp_max, target_amp, amp, synth.CutOffVolume);
                        ...
                        if (volenv_section == fluid_voice_envelope_index.FLUID_VOICE_ENVATTACK)
                        {
                            ...
                            if (volenv_val > 0)
                            {
                                ...
                                if (synth.MPTK_ApplyModLfo)
                                {
                                    ...
                                    env_value = Convert.ToInt64(-((-200f * Mathf.Log(vol) / Mathf.Log(10f) - lfo) / 960f - 1));
                                }
                                else
                                {
                                    env_value = Convert.ToInt64(-((-200f * Mathf.Log(volenv_val) / Mathf.Log(10f)) / 960f - 1));
                                }
        ...
        */
        ...
        {
            if (!weakDevice)
                Debug.LogFormat("VolEnv - [{0,4}] {1,-15} TimeFromStart:{2} Delta:{3:F2} section:{4} volenv_val:{5:0.000} volenv_count:{6} incr:{7:0.0000} coeff:{8:0.00}",
