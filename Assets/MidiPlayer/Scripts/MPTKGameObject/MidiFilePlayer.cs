    ...
    public partial class MidiFilePlayer : MidiSynth
    {
                ...
                if (MidiPlayerGlobal.MPTK_SoundFontLoaded)
                {
                    ...
                    else if (!MPTK_IsPlaying)
                    {
                        ...
                        if (MidiPlayerGlobal.ImSFCurrent != null && MidiPlayerGlobal.CurrentMidiSet != null && MidiPlayerGlobal.CurrentMidiSet.MidiFiles != null && MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count > 0)
                        {
                            ...
                            if (MPTK_CorePlayer)
                                Routine.RunCoroutine(ThreadCorePlay(null).CancelWith(gameObject), Segment.RealtimeUpdate);
                            else
        ...
        public IEnumerator<float> ThreadCorePlay(byte[] midiBytesToPlay = null, float fromPosition = 0, float toPosition = 0)
        {
            ...
            try
            {
                ...
                miditoplay.KeepNoteOff = MPTK_KeepNoteOff;
                miditoplay.EnableChangeTempo = MPTK_EnableChangeTempo;
                if (!miditoplay.MPTK_Load(midiBytesToPlay))
                    miditoplay = null;
#if DEBUG_START_MIDI
            ...
            if (miditoplay != null)
            {
                ...
                //miditoplay.ReadyToPlay = true; //miditoplay.ReadyToStarted = true; //V2.84 move below
                ...
                do
                {
                    // v2.86 - move before the call to OnEventStartPlayMidi
                    ...
                    volumeStartStop = 1f;
                    IdSession++;
                    miditoplay.ReadyToPlay = true; //miditoplay.ReadyToStarted = true; //V2.84 from below
                    ...
                    do
                    {
                        ...
                        if (needDelayToStart && delayNeedToStopSecond > 0f)
                        {
                            ...
                            //Debug.Log($"{DateTime.Now.ToLongTimeString()} {timeAtNeedToStopSecond - Time.realtimeSinceStartup} {delayNeedToStopSecond} {pct}");
                            if (pct > 0f)
