...
namespace MidiPlayerTK
{
    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        /// <summary>
        /// Multiplier to increase or decrease the default release time defined in the SoundFont. Recommended values between 0.1 and 2. Default is 1 (no modification of the release time)
        /// </summary>
        [Tooltip("Modify the default value of the release time")]
        [Range(0.1f, 2f)]
        public float MPTK_ReleaseTimeMod = 1f;

        [Tooltip("Manage a buffer of Voice to enhance performance")]
        ...
        /// <summary>
        /// If true then Midi events are read and play from a dedicated thread. If false, MidiSynth will use AudioSource gameobjects to play sound.
        /// This properties must be defined before running the application from the inspector. The default is true. 
        /// The non core mode player will be removed with the next major version (V3)
        /// </summary>
        [HideInInspector]
        ...
        public IEnumerator<float> ThreadLeanStartAudio(AudioSource audioSource)
        {
            ...
            while (audioSource.volume < 1f)
            {
                ...
                yield return Routine.WaitForSeconds(0.01f);
            }
            yield return 0;
        }
        ...
        /// <summary>
        /// Initialize the synthetizer: channel, voices, modulator. It's not usefull to call this method if you are using prefabs (MidiFilePlayer, MidiStreamPlayer, ...).
        ...
        /// <summary>
        /// Start the Midi sequencer: each midi events are read and play in a dedicated thread. This thread is automatically stared 
        /// by prefabs MidiFilePlayer, MidiListPlayer, MidiExternalPlayer.
        /// </summary>
        ...
        protected void ResetMidi()
        {
            ...
            watchMidi.Reset();
            watchMidi.Start();
            if (miditoplay != null) miditoplay.StartMidi();
        }
...
#if DEBUG_PERF_AUDIO
            StatDspLoadMA = new MovingAverage(300);
            //StatDspLoadLongMA = new MovingAverage(3000);
            StatAudioFilterReadMIN = double.MaxValue;
            StatAudioFilterReadMAX = 0;
            StatAudioFilterReadMA = new MovingAverage(300);
            StatSampleWriteMA = new MovingAverage(300);
            StatProcessListMA = new MovingAverage(300);
#endif
#if DEBUG_PERF_MIDI
            StatDeltaThreadMidiMIN = double.MaxValue;
            StatDeltaThreadMidiMAX = 0;
            StatDeltaThreadMidiMA = new MovingAverage(300);
        ...
        /// <summary>
        /// Set forced preset on the channel. Midi will allways playing with this preset even if a Midi Preset Change message is received . Set to -1 to disable this behavior.
        /// </summary>
        ...
        public fluid_voice fluid_synth_alloc_voice(HiSample hiSample, int chan, int _idSession, int key, int vel)
        {
            ...
            else
            {
                ...
                MoveVoiceToFree();
                if (MPTK_AutoBuffer)
                    AutoCleanVoice(DateTime.Now.Ticks);
            }
        ...
        /// <summary>
        ///V2.86 Play immediately one Midi event. Like MPTK_PlayEvent but take time to process the event before returning to the caller.
        ...
        private void WriteAllSamples(long ticks, float[] reverb_buf, float[] chorus_buf)
        {
            ...
            {
                fluid_voice voice = ActiveVoices[i];
                //Debug.Log("voice.TimeAtStart :" + voice.TimeAtStart + " System.DateTime.Now.Ticks:" + System.DateTime.Now.Ticks);
                try
                {
